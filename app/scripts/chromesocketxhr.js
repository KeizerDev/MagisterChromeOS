...
ChromeSocketXMLHttpRequest.prototype = {
    open: function(method, url, async) {
        ...
        console.assert(this.uri.protocol == 'http') // https not 
supported for chrome.socket yet
    },
    setRequestHeader: function(key, val) {
        this.extraHeaders[key] = val
    },
    send: function(data) {
        console.assert( ! data ) // do not support sending request body 
yet
        chrome.sockets.tcp.create({}, _.bind(this.onCreate, this))
        if (this.timeout !== 0) {
            this.timeoutId = setTimeout( _.bind(this.checkTimeout, 
this), this.timeout )
        }
    },
    createRequestHeaders: function() {
        ...
        var headers = {'Connection': 'close',
                       'Accept-Encoding': 'identity', // servers will 
send us chunked encoding even if we dont want it, bastards
//                       'User-Agent': 
'uTorrent/330B(30235)(server)(30235)', // setRequestHeader /extra header 
is doing this
                       'Host': this.uri.host}
...
HTTP/1.1')
    ...
    onCreate: function(sockInfo) {
        ...
        chrome.sockets.tcp.connect( sockInfo.socketId, this.getHost(), 
this.getPort(), _.bind(this.onConnect, this) )
    },
    ...
    writeFromBuffer: function() {
        ...
        this.writing = true
        var data = 
this.writeBuffer.consume_any_max(jstorrent.protocol.socketWriteBufferMax)
        //console.log('writing data',ui82str(data))
        chrome.sockets.tcp.send( this.sockInfo.socketId, data, 
_.bind(this.onWrite,this) )
    },
    ...
    doRead: function() {
        ...
        //chrome.sockets.tcp.read( this.sockInfo.socketId, 
jstorrent.protocol.socketReadBufferMax, _.bind(this.onRead,this) ) // 
new api dont work this way
    },
    close: function() {
        this.closed = true
        if (this.sockInfo) {
    ...
    onRead: function(result) {
        console.log('onread',result.data.byteLength, [ui82str(new 
...
function(c){return c.charCodeAt(0)}))
            if (idx != -1) {
                // not sure what encoding for headers is exactly, latin1 
or something? whatever.
                var headers = ui82str(new Uint8Array(data, 0, idx + 4))
                console.log('found http tracker response headers', 
headers)
